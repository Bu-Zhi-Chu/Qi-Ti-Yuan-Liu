    ...
    if (currentId) {
        ...
        if (chartInst && typeof chartInst.getOption === 'function') {
            try {
                ...
                instanceSeriesData = series.filter((s) => Array.isArray(s?.data)).map((s) => s.data)
...
**解决**：在 `ECharts.svelte` 的 `$derived(option)` 阶段追加补全逻辑（含饼图规则）：

-   **触发条件**：仅当 `dataSource === 'json'` 且 `codeResult.series.length > 0` **且用户提供了 `legend` 对象**（即使是空对象 `{}`）但 `legend.data` 缺失或为空数组时生效；
-   **补全优先级**：
    -   ① 饼图优先：若检测到饼图（`series.type === 'pie'` 或 `series.data` 中存在对象项且含 `name` 字段），从 `series[].data[].name` 采集去重后的名称序列，填充 `legend.data`（日志：`自动填充 legend.data(饼图)`）；
    -   ② 其他图表：回退为 `legend.data = series.map(s => s.name || '')`（日志：`自动填充 legend.data(系列名)`）。
...
-   当 `legend.data` 更新后，仅在「不存在饼图系列」且「`legend.data.length === series.length`」时，同步更新 `series[i].name = legend.data[i]`；否则跳过同步，避免将饼图的图例项误写入系列名。

**效果**：切换页签或刷新后，图例与系列保持合理同步；饼图场景下，图例项来源于数据项的 `name`，系列名保持原有含义（如“访问来源”），浏览器控制台不再出现图例警告。同时当用户不希望显示图例时（即不提供 `legend` 属性），图表不会显示任何图例。
...
-   legend 自动补全：`json` 模式下，若用户提供了 `legend` 对象但 `legend.data` 缺失或为空，自动以 `series[].name` 填充；若用户已提供非空 `legend.data`，严格尊重不覆盖。
-   名称同步：当 `legend.data` 补全或存在时，使用 `legend.data[i]` 同步更新 `series[i].name`，保持图例与系列一致。
-   多轴替换：`executeJavaScriptCode` 支持替换 `series[].data`、`xAxis.data`、`yAxis.data`，并移除旧版截断/填充逻辑，保持数据原始长度。
-   键下发：`real/mock` 拉取成功后下发字段到 `dataMappingKeysStore`，供数据面板的下拉选择使用。
-   映射 UI：`mock/real` 映射行在 `seriesCount > 0` 时渲染；若存在 `dataMappingKeys` 则提供下拉选择，否则回退到文本输入。

### 12.2 修正与补充

-   缓存位置修正：缓存由 `DataEditor.svelte` 组件层实现（通过 `lastCode/lastExtraction` 避免重复解析），而非 `series-extractor.service.ts` 内部缓存。
-   首次渲染写回 `seriesData`：当解析成功但节点属性缺少 `seriesData` 时，`DataEditor.svelte` 在首次渲染通过 `$effect` 将解析结果写回到节点属性，避免映射面板出现空数据。
-   兼容“伪 RegExpMatchArray”：为统一处理匹配结果，提取器在使用实例数据时会构造形状兼容的对象，确保后续代码可按正则匹配数组一致方式消费。
-   JSON 覆盖规则：当提供 `seriesData` 且存在映射时，允许覆盖 `legend.data`、`xAxis.data` 与 `series[i].data`；显式配置优先于默认推断。
-   动态表格映射：`DynamicTable` 在 `json` 模式下使用 `bodyDataCode` 解析，字段源自 `jsonMappingKeys` 与 `tableHeaders`，并通过 `jsonColumnMapping` 记录列映射选择。
-   `seriesCount` 更新策略：依赖 `derivedStateResult.matches.length` 变更，并避免瞬时 0 值导致 UI 抖动（结合局部状态与日志门限）。
-   加载/错误占位：在 `real/mock` 模式，`$derived(option)` 提供 loading/error 的占位渲染，防止空白图表并提升可用性。

### 12.3 代码参考位置

-   `DataEditor.svelte`：`derivedState`、`seriesCount` 管理、首次写回 `seriesData` 的 `$effect`、`updateMockSeriesMapping`/`updateRequestSeriesMapping`、`DynamicTable` JSON 映射。
-   `ECharts.svelte`：`executeJavaScriptCode` 的数据替换、legend 自动补全与 `series[i].name` 同步、`processedData()` 生成映射数据、拉取后下发 `dataMappingKeysStore`。
-   `series-extractor.service.ts`：实例数据优先提取、匹配对象构造、legend 解析与同步、系列数量与写回判断。
