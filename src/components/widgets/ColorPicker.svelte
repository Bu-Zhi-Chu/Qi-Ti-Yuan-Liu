...
<script lang="ts">
    ...
    function getColorsFromDomTree(node: any, acc: string[] = []) {
        ...
        // 收集边框颜色
        const borderColors = [styles.borderColor, styles.borderTopColor, styles.borderRightColor, styles.borderBottomColor, styles.borderLeftColor].filter((color) => color && color !== 'transparent' && color !== 'rgba(0, 0, 0, 0)')
