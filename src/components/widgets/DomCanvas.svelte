...
<script lang="ts">
     ...
     */
    $effect(() => {
        if (!editing && (offsetX !== 0 || offsetY !== 0)) {
            offsetX = 0
            offsetY = 0
        }
    })
...
<div
    ...
    class:editing
    use:usePan={{ key: 'Space', onPan: handlePan, scaleAccessor: getScaleRatio }}
    style={`width: 100%; height: 100%; transform: ${editing ? `translate(calc(-50% + calc(${offsetX}px * var(--scale-ratio, 1))), calc(-50% + calc(${offsetY}px * var(--scale-ratio, 1)))) scale(0.5)` : `translate(calc(${offsetX}px * var(--scale-ratio, 1)), calc(${offsetY}px * var(--scale-ratio, 1)))`}; transform-origin: center center;`}
    role="application"
