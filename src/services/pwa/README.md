...
PWA模块为七巧板低代码工具提供完整的渐进式Web应用支持，包括环境检测、Service Worker管理、智能降级策略等功能。
...
### 开发环境
- 自动跳过Service Worker注册（避免开发干扰）
- 显示详细的控制台调试信息
- 支持HTTP协议（降级模式）

### 生产环境
- 自动注册Service Worker
- 使用vite-plugin-pwa生成优化的Service Worker
- 要求HTTPS协议（PWA标准）

### 降级策略
当环境不支持PWA时：
- 自动跳过Service Worker注册
- 移除PWA专属UI元素
- 保持普通Web应用功能
- 在控制台输出友好提示
...
2. **Application > Manifest**: 验证manifest配置
3. **Console**: 查看PWA初始化日志
...
- [vite-plugin-pwa文档](https://vite-pwa-org.netlify.app/)
