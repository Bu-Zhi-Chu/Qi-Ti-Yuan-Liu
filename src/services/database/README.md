...
`dexie-service.ts` 是基于 Dexie.js 的 IndexedDB 封装服务，为七巧板项目提供完整的数据持久化解决方案。支持项目、组件、历史记录等核心数据的增删改查操作。
...
const components = [
  {
    pageId: "home",
    type: "button",
    x: 150, y: 350, width: 120, height: 40, zIndex: 2,
    properties: { text: "立即开始", variant: "primary" }
  },
  {
    pageId: "home",
    type: "image",
    x: 500, y: 200, width: 400, height: 300, zIndex: 1,
...
- **索引优化**: 基于pageId、type等字段建立索引
- **批量操作**: 使用bulkAdd、bulkDelete等批量API
...
3. **存储限制**: IndexedDB存储空间取决于浏览器和磁盘空间
4. **数据安全**: 敏感数据建议加密后存储
5. **备份策略**: 定期使用backup()方法创建数据备份

## 更新日志

### v1.0.0
- 初始版本发布
- 支持项目、组件、历史记录的完整CRUD操作
- 添加数据导出导入功能
- 支持批量操作和分页查询